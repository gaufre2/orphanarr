...
describe("Hard linking check", () => {
  test("2 files are hard linked (from path)", async () => {
    const torrentFile = await FileExtended.fromPath(join(targetDir, "file1"));
    expect(await torrentFile.isHardLinkedWith(join(linkDir, "file1a"))).toBe(
      true
    );
  });
  ...
  test("2 files are not hard linked", async () => {
    const torrentFile = await FileExtended.fromPath(join(targetDir, "file1"));
    expect(await torrentFile.isHardLinkedWith(join(linkDir, "file3a"))).toBe(
      false
    );
  });
  ...
  test("1 file is invalid", async () => {
    const torrentFile = await FileExtended.fromPath(join(targetDir, "file1"));
    expect(
      torrentFile.isHardLinkedWith(join(linkDir, "invalid"))
    ).rejects.toThrow();
  });
});
